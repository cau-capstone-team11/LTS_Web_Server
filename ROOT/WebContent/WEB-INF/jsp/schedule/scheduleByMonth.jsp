...
<script>
	...
	function registerPlan() {
		var title = $("#register tr").eq(0).children().eq(1).children().eq(0)
				.val();

		if (title == "")
			alert("Title is Empty");
		else {
			var description = $("#register tr").eq(1).children().eq(1)
					.children().eq(0).val();
			var date = $("#register tr").eq(2).children().eq(1).html();
	...
	function loadPlan(info) {
		...
		if ($("#content").css("display") == "none") {
			...
			$("#register tr").eq(2).children().eq(1).html(
					dateFormat(info.event.start));
		} else {
			$("#register tr").eq(0).children().eq(1).children().eq(0).val(
					info.event.title);
			$("#register tr").eq(1).children().eq(1).children().eq(0).val(
	...
	function editPlan() {
		if (currentEvent != null) {
			...
			var title = $("#register tr").eq(0).children().eq(1).children().eq(
					0).val();
			var description = $("#register tr").eq(1).children().eq(1)
					.children().eq(0).val()
...
<section class="w3ls-bnrbtm py-5" id="about">
	<div class="container py-xl-5 py-lg-3">
		<h3 class="title-w3 mb-md-5 mb-sm-4 mb-2 text-center font-weight-bold">Holiday Management</h3>
